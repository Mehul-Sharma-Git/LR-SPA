...
export function VerifySMSOTP() {
  ...
  useEffect(() => {
    const requestOTP = async () => {
      const response = await authService.requestSMSOTP();
      console.log(response);
      if (response.IsPosted) {
        toast.success("SMS OTP sent successfully");
        navigate("/verify-sms-otp");
      } else {
  ...
  const onSubmit = async (data: VerifyOTPFormData) => {
    setIsLoading(true);
    try {
      const response = await authService.verifySmsOTP(data.code);
